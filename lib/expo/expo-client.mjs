...
import { listBuilds, listChannels } from "./eas-cli.mjs";
...
export async function buildExpoUsageReport({ startDate, endDate } = {}) {
  ...
  if (cache.payload && cache.key === cacheKey && now - cache.at < CACHE_MS) {
  ...
  try {
    ...
    const [buildsResult, channelsResult, insightsResult, observeResult, observeEventsProbe, updateInsightsResult] = await Promise.all([
      listBuilds(project.projectId, 15),
      listChannels(project.projectId, 25),
      fetchAllChannelInsights(project.projectId, { days, startDate, endDate }),
      fetchObserveMetricsSummary(project.projectId),
      fetchObserveEvents(project.projectId),
      fetchRecentUpdateInsights(project.projectId, { days, startDate, endDate, limit: 5 }),
    ]);
