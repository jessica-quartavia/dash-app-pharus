...
export function fetchRecentUpdateInsights(projectId, { days = 30, startDate, endDate, limit = 5 } = {}) {
  const listed = listUpdateGroups(projectId, Math.max(limit, 10));
  if (!listed.ok) return { ok: false, rows: [], error: listed.error };
  const groups = normalizeUpdateGroups(listed.data, limit);
  const rows = [];
  const errors = [];
  for (const group of groups) {
    const result = fetchUpdateInsights(projectId, group.groupId, { days, startDate, endDate });
    if (!result.ok) {
      errors.push({ groupId: group.groupId, error: result.error });
      continue;
    }
    rows.push(...parseUpdateInsights(group, result.data));
  }
