...
export function discoverChannelRuntimePairs(projectId) {
  ...
  for (const channel of channelsResult.rows || []) {
    const view = viewChannel(projectId, channel.name);
  ...
  if (!hasProduction) {
    const buildsResult = listBuilds(projectId, 5);
...
export function fetchChannelInsights(projectId, { channel, runtimeVersion, days = 30, startDate, endDate }) {
  ...
  const result = runEas(args, { cwd, expectJson: true, timeoutMs: 180_000 });
...
export function fetchAllChannelInsights(projectId, { days = 30, startDate, endDate } = {}) {
  ...
  for (const pair of discovered.pairs) {
    ...
    if (!fetched.ok) {
      results.push({ ...pair, ok: false, error: fetched.error });
      continue;
    }
    results.push({
      ...pair,
