...
import { buildJourneySpeedSource, logJourneySpeedDebug } from "./journey-speed.mjs";
...
export function buildJourneyDataset(options = {}) {
  return cached("journey-v2", async () => {
    ...
    const [progress, stages, scheduled, catalog] = await Promise.all([
      ...
      dataRestFetchAll("scheduled_meetings", "id,user_id,meeting_id,start_time,end_time,status", { schema: "core" }),
      dataRestFetchAll("meetings", "id,meeting_title,meeting_slug,order,is_active", { schema: "core" }),
    ]);
    const journeySpeedSource = buildJourneySpeedSource({
      users: clientsDataset.clients,
      meetings: scheduled,
      catalog,
    });
    logJourneySpeedDebug(journeySpeedSource, scheduled);
    return {
      clients: clientsDataset.clients,
      advisors: clientsDataset.advisors,
      progress: officialRows(progress, index),
      stages: officialRows(stages, index),
      journeySpeedSource,
      source: {
        schemas: ["auth", "core", "metrics"],
        tables: ["auth.users", "core.user_progress", "core.scheduled_meetings", "core.meetings", "metrics.v_current_stage"],
      },
    };
  }, options);
}
...
export function buildUsageContextDataset(options = {}) {
  return cached("usage-context", async () => {
    ...
    return { available: true, officialClients: dataset.populationTotal, kpis: [
      ...
      { key: "meetings", label: "Com reuniões", value: count((client) => client.hasMeetings) },
