...
import { fetchCoreColumns, fetchOfficialUsers, fetchBackofficeColumnsSafe } from "./queries.mjs";
...
export async function buildMechanismsDataset() {
  ...
  const [catalogRows, userMechanisms, profiles, allocations, internalProfiles] = await Promise.all([
  ...
  const advisorMap =
    allocations.ok && internalProfiles.ok
