...
export const METRICS_DOCUMENTATION = [
  ...
  page("utilizacao-app", "Utilização do App", "Mostra utilização e comportamento da plataforma pelo Google Analytics, com informações técnicas do Expo/EAS.", [
    ...
    metric("versoes", "Versões", "Versões técnicas do aplicativo encontradas no Expo/EAS.", "Mostra versões e plataformas do EAS Observe (eas observe:metrics-summary). Em produção a página lerá o snapshot persistido; até a sincronização, informa que os dados aguardam coleta. Não substitui os usuários ativos do Google Analytics."),
    ...
    metric("observe", "Saúde e performance", "Eventos técnicos de inicialização, renderização e carregamento do aplicativo.", "Usa o mesmo resumo do EAS Observe por plataforma e versão. Na Vercel a leitura futura é o snapshot persistido; até a sincronização, a página informa que os dados aguardam coleta. Eventos anônimos não são usuários da base Pharus."),
